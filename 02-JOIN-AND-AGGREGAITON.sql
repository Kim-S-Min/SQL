...
-- 일반적으로는 이런 결과를 원하지 않을 것이다
-- 첫 번째 테이블의 department_id 정보와 두 번째 테이블의 department_id를 일치시키는것
SELECT employees.employee_id, employees.first_name, employees.department_id,
    departments.department_id, departments.department_name
FROM employees, departments
WHERE employees.department_id = departments.department_id;  -- 두테이블의 연결정보 조건을 부여한다
-- INNER JOIN, EQUI JOIN

-- 컬럼명의 모호성을 피하기 위해 테이블명.컬럼명
-- 테이블의 별명(alias)를 붙여주면 편리
SELECT employee_id, first_name, -- 컬럼 명ㅇ ㅣ소속이 명확하면 테이블 명은 명시하지 않아도 된다
    emp.department_id,
    dept.department_id,
    department_name
...
FROM employees emp JOIN departments dept
                ON (emp.department_id = dept.department_id);    -- ON -> JOIN문의 WHERE절
...
-- ------------
SELECT * FROM jobs WHERE job_id = 'AD_ASSY'; -- min : 3000, max : 6000
...
SELECT first_name,
    ...
    department_id
FROM employees emp, departments dept
WHERE emp.deaprtment_id (+) = dept.department_id (+);   -- FULL OUTER JOIN 은 불가능 하다
*/
...
-- SELF JONI : 자신의 FK가 자신의 PK를 참조하는 방식의 JOIN
-- 자신을 두번 호출하므로 alias 사용할 수 밖에 없는 JOIN
...
SELECT COUNT(*) FROM employees; -- *는 전체 레코드 카운트를 집계(내부값이 null이 있어도 집계)
...
-- 사원들이 받는 평균 커미션 비율
SELECT AVG(commission_pct) FROM employees;  -- 받지않는 사람들은 제외됨
...
SELECT department_id, AVG(salary)
FROM employees
WHERE AVG(salary) >= 2000 -- error : 이 시점에서는 AVG(salary)가 수행되지 않은 상태 -> 없다
GROUP BY department_id;
-- Error : 집계 작업이 일어나기 전에 WHERE절이 실행되기 때문이다
...
-- ROLLUP
-- GROUP BY 절과 함께 사용
-- GROUP BY의 결과에 좀더 상세한 요약을 제공하는 기능을 수행한다(Item Subtotall)
-- 부서별 급여의 합계 추출(부서 아이디 ,job_id) - 1차 group 2차 group
...
-- 집합 연산에 관련된 IN, ANY, ALL, EXSIST등을 이용해야 한다
...
FROM employees emp, (SELECT department_id, MAX(salary) salary
                        FROM employees
                        GROUP BY department_id) sal -- 임시테이블을 만들어서 sal 별칭부여
...
-------------------
-- 2007년 입사자 중에서 급여 순위를 5위까지 뽑아봅시다
...
ORDER BY salary DESC;   -- rownum이 정해진 이후 정렬을 수행한다
...
SELECT salary, first_name,
    ...
    DENSE_RANK() OVER (ORDER BY salary DESC) as densc_rank, -- 중복순위 상관 없이 다음 순위 부여
    ROW_NUMBER() OVER (ORDER BY salary DESC) as row_number, -- 중복 여부 관계없이 차례대로 순위 부여
    rownum -- 정렬되기 이전의 레코드 순서
FROM employees;

-- Hierarchical Query : 트리형태의 구조를 추출한다
...
-- 연습문제 : 위 트리 구조에서 매니저의 일므도 출력해 주세요